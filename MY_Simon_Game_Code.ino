 ...
 *
 * Simon Game ported for the ATmega328
 ...
 * Select the - Lilypad for arduino - board, when programming with ftdi connector
 * Power up with AA's and turn on - then program it.
 ...
 * 
 * Modified October 2013 by Stephen O'Gara to add solenoid activation upon winning the game.
 * Lengthened the winner sounds and lights series.
 */
...
int solenoid1 = A0;                 // solenoid1 connected to analog pin A0
...
/* Change this number to around 12 when ready for final adjustments  */
...
winner_sound(void)
{
  ...
  /* changed loop up to 9 times for Y< , then changed it back */
...
/* added in the digital write lines for solenoid1 on pin A0*/
...
play_winner(void)
{
  ...
  /* Added in this solenoid output with a 2 second hold */
  
  digitalWrite(solenoid1, HIGH);   // sets the solenoid on
  delay_ms(2000);                  // waits for 9 seconds
  digitalWrite(solenoid1, LOW);    // sets the solenoid off
  delay_ms(3000);                  // waits for a second
/* need to turn off the lights around here. Then it would be great if the sleep mode could be used to reduce power */  
 /* Turn off all LEDs */
  set_leds(0);
...
/* change pin mode for solenoid to an output on analog pin AO */
/* add the pin mode statement here*/



void setup()
{
  pinMode(solenoid1, OUTPUT);      // sets the analog pin as output
}
...
void loop()
{
 ...
 /* removed play_winner(); from right here*/
